...
local function AddMoney(accountName, amount, reason)
    if not reason or reason == '' then reason = 'Bank Deposit' end
    local newStatement = {
        amount = amount,
        reason = reason,
        date = os.time() * 1000,
...
local function RemoveMoney(accountName, amount, reason)
    if not reason or reason == '' then reason = 'Bank Withdrawal' end
    local newStatement = {
        amount = amount,
        reason = reason,
        date = os.time() * 1000,
...
QBCore.Functions.CreateCallback('as-banking:server:openBank', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('as-banking:server:openATM', function(source, cb)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return end
...
QBCore.Functions.CreateCallback('as-banking:server:withdraw', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
...
QBCore.Functions.CreateCallback('as-banking:server:deposit', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('as-banking:server:internalTransfer', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local job = Player.PlayerData.job
...
QBCore.Functions.CreateCallback('as-banking:server:externalTransfer', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local job = Player.PlayerData.job
...
QBCore.Functions.CreateCallback('as-banking:server:orderCard', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    ...
    exports['qb-inventory']:AddItem(src, 'bank_card', 1, false, info, 'as-banking:server:orderCard')
    cb({ success = true, message = Lang:t('success.card') })
end)

QBCore.Functions.CreateCallback('as-banking:server:openAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('as-banking:server:renameAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateCallback('as-banking:server:deleteAccount', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
    local accountName = data.accountName
...
QBCore.Functions.CreateCallback('as-banking:server:addUser', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateCallback('as-banking:server:removeUser', function(source, cb, data)
    local src = source
    local Player, citizenid = getPlayerAndCitizenId(src)
    if not Player or not citizenid then return cb({ success = false, message = Lang:t('error.error') }) end
...
QBCore.Functions.CreateUseableItem('bank_card', function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        TriggerClientEvent('as-banking:client:useCard', source)
    end
end)
